...
class ListHelper
{
    ...
    public static function popular_items($days = 7, $limit = 15, $shop_id = null)
    {
        ...
        // It's a tric for the demo only to get different items
    ...
    {
        if ($brands = get_from_option_table('featured_brands', [])) {
    ...
    {
        if ($vendors = get_from_option_table('featured_vendors', [])) {
    ...
    {
        return Manufacturer::withSum('inventories', 'sold_quantity')
            ->orderByDesc('inventories_sum_sold_quantity')
            ->limit(8)
            ->get();
    }
    ...
    {
        return Category::with('featureImage')
    ...
    //         $join->on('images.imageable_id', '=', 'cart_items.inventory_id')->where('images.imageable_type', '=', 'App\Models\Inventory');
    //     })
