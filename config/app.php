    ...
    if (file_exists($manifest = $dir . '/manifest.json')) {
        if ($json = file_get_contents($manifest)) {
            ...
            if (isset($data['provider'])) {
                ...
                // $arr = explode('\\', $data['provider']);
                // $path = base_path('packages\\' . $data['dir'] . '\\src\\' . end($arr) . '.php');
...
return [
    ...
    */
    ...
    | This key should be set to a random, 32 character string, otherwise
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | Callback API callback URL.
    |--------------------------------------------------------------------------
    |
    | This URL is used by the system as callback base URL for all third party APIs
    | Callback API callback url is useful when testing APIs on local.
    | For production this will not be in use.
    | e.g. https://caf6-27-147-204-114.ap.ngrok.io
    |
    */

    'callback_base_url' => env('APP_ENV') == 'local' ? env('API_TEST_REDIRECT_URL') : env('APP_URL'),
