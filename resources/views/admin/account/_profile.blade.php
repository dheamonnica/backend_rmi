<div class="row">
  ...
  {{-- <div class="col-md-3">
    ...
    @if ($profile->isFromMerchant())
      <div class="form-group">
        @if(isset($profile->shop->pay_to))
          <a class="ajax-modal-btn btn btn-default" href="javascript:void(0)" data-link="{{ route('admin.account.shop.editAccountNumber') }}"><i class="fa fa-lock"></i> {{ trans('app.update_account') }}</a>
        @else
          <a class="ajax-modal-btn btn btn-default" href="javascript:void(0)" data-link="{{ route('admin.account.shop.editAccountNumber') }}"><i class="fa fa-lock"></i> {{ trans('app.add_account') }}</a>
        @endif
      </div>
    @endif
