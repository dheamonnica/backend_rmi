<script type="text/javascript">
  ...
  function initDatatables() {
    ...
    function handleFilterChange() {
      ...
      var filteredUrl = "{{ route('admin.order.bulkorder_process', ['paymentStatus' => '0', 'orderStatus' => '0']) }}";
      filteredUrl = filteredUrl.replace(/\/[^/]+\/[^/]+$/, `/${paymentStatusFilter}` + `/${orderStatusFilter}`); // replaces the last two parameters of the url
    ...
    $('#filter-all-order-table-order-status, #filter-all-order-table-payment-status').on('change', handleFilterChange);
